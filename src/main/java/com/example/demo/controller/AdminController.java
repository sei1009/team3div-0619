...
public class AdminController {
	...
	public ResponseEntity<Void> deleteUser(@PathVariable Long id) {
		...
		if (userOptional.isPresent()) {
			...
			// 1. ユーザーに紐づく勤怠レコードを削除
			List<Attendance> userAttendances = attendanceDao.findByUsersId(id);
			attendanceDao.deleteAll(userAttendances); // 紐づく勤怠レコードを一括削除

			// 2. ユーザーに紐づく申請レコードを削除
			List<Request> userRequests = requestDao.findByUsersId(id);
	...
	public ResponseEntity<Void> deleteRequest(@PathVariable Long id) {
		...
		if (requestOptional.isEmpty()) { // Java 9+ の Optional#isEmpty()。Java 8 なら !isPresent()
			return ResponseEntity.notFound().build();
		}

		List<Attendance> associatedAttendances = attendanceDao.findByRequestId(id);

		for (Attendance attendance : associatedAttendances) {
			attendance.setRequest(null); // AttendanceからRequestへの参照を解除
		}

		requestDao.deleteById(id); // Requestを削除
		return ResponseEntity.noContent().build();
	}
	...
	public ResponseEntity<List<CombinedApplicationDto>> getAllCombinedApplications() {
		List<Attendance> attendances = attendanceDao.findAll(); // またはuser/requestと共にフェッチするカスタムクエリ
		List<CombinedApplicationDto> dtos = attendances.stream()
				.map(CombinedApplicationDto::new)
